...
На момент закрытия 23.09 коммит и push выполнены в рабочую ветку; merge в main, Google Play, GitHub Release и production deploy тогда не выполнялись. Финальный служебный commit обновляет только документацию; source commit APK выше остаётся точным идентификатором его исходников.

## Хранение артефактов после очистки — 24.09.2026

Удалены два предыдущих APK, временный дополнительный QA AVD и пересоздаваемые build/cache-каталоги. Финальный APK с SHA-256 `6880c443…`, его JSON, прежние JSON metadata и каталоги QA evidence сохранены. Перед удалением `app/build` все 179 report/result/log-файлов из его тестовых каталогов архивированы и проверены по содержимому: `build/qa-stage3-20260923/original-gradle-reports.zip`. Точный список удалённых путей и checksum архива: `build/release-close-20260923/cleanup-20260924.json`. Исторические ссылки на старые APK выше/в ранних отчётах являются идентификаторами прежних прогонов, а не текущими файлами для установки.
