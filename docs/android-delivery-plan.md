...
Last updated: 2026-09-24 (repository cleanup, EPS-09 evidence synchronized, and verified baseline integration into main)
...
- Under the backend contract in force during Stage 4.4, natural completion of a 4-second A transitioned automatically to the queued 30-second B, which was observed playing at `0:02 / 0:28` with `Pause`. A pause or seek inside the then-current 15-second completion window removed/cleared the episode server-side while Android kept showing it, so Android temporarily mirrored that boundary. Backend commit `6c0ce47` later removed this behavior; the threshold code and tests described by this historical paragraph no longer exist.
...
`APP-03`, `APP-04`, and `APP-12` intentionally remained Specified after wave 1. Their partial automated evidence passed, but a complete result required safely isolated application/backend routing for first setup plus the final production/device backup smoke check. `APP-11` also had only partial failed-logout evidence at that point and was subsequently completed in wave 14. The shared `5051` state was not reset or interrupted to manufacture wave-1 evidence. Full gate: 94 unit tests, 44/44 connected tests, debug/release lint, debug/test assemblies, and minified release assembly. Lint was run sequentially after release generation because a combined parallel Gradle invocation hit an Android Lint/Hilt generated-source race; the sequential lint checks passed.
...
Scenario wave 8 completed on 2026-07-21 for the contract then in force for `PLY-11`, `PLY-13`, and `PLY-19`; `PLY-09` still required its later physical-phone pass. A temporary authenticated 20/60/20-second MP3 podcast exercised online auto-next, final-item completion, and app-only network denial. During the outage, B was already playing from buffer while backend still reported active A and A's explicit completed update was durable in `PendingPlaybackSync`; after recovery it could not hijack B. Completing sole C exposed and fixed a separate stale-Home defect, and Home immediately showed `Playlist is empty`. The old paused-threshold reconciliation details from this wave were removed after backend commit `6c0ce47`; delayed explicit-completion persistence and non-hijacking evidence remain applicable. Full historical gate: 103 unit tests, 83/83 connected tests, debug/release lint, debug APK, and minified release APK.
...
Scenario wave 12 completed on 2026-07-22 for `PLY-05`, with emulator evidence added but physical-phone acceptance still pending for `PLY-01`, `PLY-02`, and `PLY-04`. Real Planet Money Play/Pause/resume and authoritative position updates passed on the current test APK. The required gesture audit found a functional drag defect: `PlayerView` sent `onSeekTo` for the initial touch and every pointer movement, producing a serialized burst of playback writes; the UI reached the selected position while backend could retain a stale intermediate value. Drag now has a local preview and dispatches only the final position after release, while tap also dispatches once. Connected tests assert the single-call contract. On Pixel 9 a tap stored `1283`, then a backward drag stored the final `684`; before the fix the same drag left the prior backend position. Rewind 10 from backend `51` correctly displayed local `41`, backend retained `51` under its sub-30-second rule, and force-stop/relaunch restored `51` without autoplay. The original backend position was restored and the crash buffer was empty. Full gate: 109 unit tests, 99/99 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 92/127 Verified (72%).

Scenario wave 13 completed on 2026-07-26 for `SUB-01`, with emulator evidence added but physical-phone acceptance still pending for `REL-07`. A cold login against real `5051` visibly entered `Loading subscriptions` with no Refresh, Unsubscribe, Mark all listened, or episode actions before resolving to the authoritative library; connected coverage independently pins the same non-mutating loading contract. Large-list regression now creates 80 rows, semantically scrolls to the final long title, and proves the selected action keeps episode ID `1080`. On Pixel 9, the 350-row unlistened Planet Money list was deeply scrolled to the long-titled episode ID `73`; its options remained reachable, Add to playlist produced `[16,18,26,73]`, the same row switched to Remove from playlist, and removal restored `[16,18,26]`. No application defect was found, no unrelated backend state changed, emulator network conditioning was restored to normal, and the crash buffer was empty. `REL-07` stays `Specified` until the agreed final physical-phone pass. Full gate: 109 unit tests, 100/100 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 93/127 Verified (73%).

Scenario wave 14 completed on 2026-07-26 for `APP-11`. Connected launch tests cover both a backend `503` and a refused logout connection. Two immediate Logout calls produce exactly one HTTP mutation, the application enters the unavailable state instead of inventing logout success, and an authoritative authenticated-session response restores the shell after the isolated test server returns. On Pixel 9 against real `5051`, connectivity was removed only from the emulator after authenticated Settings loaded. Logout showed `mpod is not reachable`, never showed Login, retained the persisted cookie store, and exposed Retry. Restoring connectivity and tapping Retry returned directly to the authoritative Subscriptions screen without credentials; no backend or library state was changed, emulator networking was restored, and the crash buffer was empty. No production-code defect was found. Full gate: 109 unit tests, 101/101 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 94/127 Verified (74%).

Scenario wave 15 completed on 2026-07-26 for `SUB-02`. A connected ViewModel regression starts with a structured initial `503`, proves that no library is invented, retries the exact load, and recovers the authoritative podcast/episode state; existing Compose coverage performs the visible `Try again` gesture. On Pixel 9, a host-only HTTP proxy passed authentication and unrelated traffic to real `5051` but returned `503 Controlled backend failure` only for the initial `GET /api/podcasts`. Subscriptions showed the exact error and a single `Try again` action without mutation controls. After the proxy returned to passthrough, the real tap loaded the authoritative one-podcast Planet Money library without restarting the app or changing backend state. The system proxy was cleared, direct backend connectivity restored, and the crash buffer was empty. No production-code defect was found; `SUB-14` remains separate because it requires an isolated two-podcast fixture with only one episode endpoint failing. Full gate: 109 unit tests, 102/102 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 95/127 Verified (75%).

Scenario wave 16 completed on 2026-07-26 for `APP-03`, `APP-04`, `SUB-03`, and `HOM-02`. The actual parent-project backend ran against a clean temporary SQLite database on isolated port `15051`; a host-only proxy routed only the test APK's expected `5051` address to it, leaving shared state untouched. Its real session response reported `setupRequired:true`, and Pixel 9 showed Setup without a Login substitute. One Create account gesture produced exactly one registration request, one user, one session, a persisted cookie, and authenticated Subscriptions. The isolated database remained at zero podcasts. Both Subscriptions and Home displayed the distinct no-podcast state, and their Add RSS feed and Import OPML controls opened the correct modal modes without submission. Connected coverage now immediately invokes Register twice and requires one HTTP request; existing Compose tests cover both empty-state gestures on each screen. The isolated backend and proxy were stopped, its app data cleared, direct `5051` connectivity restored, and the crash buffer was empty. Full gate: 109 unit tests, 102/102 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 99/127 Verified (78%).

Scenario wave 17 completed on 2026-07-26 for `SUB-04` and `SUB-14`. A fresh isolated backend imported two locally served RSS feeds. A host-only proxy returned a controlled `503` only for Alpha's episode endpoint while all other requests reached that backend. Pixel 9 kept the failure scoped to Alpha; Beta and its real episode remained reachable and usable by swiping. Restoring passthrough and tapping the visible Try again removed both error surfaces and restored Alpha without discarding Beta. A new connected ViewModel regression preserves that partial state and exact recovery. Both podcasts were then authoritatively marked listened. A force-stop/relaunch showed `All caught up` rather than the no-podcast state; Show all restored the listened Alpha episode and changed the header action to Show unlistened. Existing unit/Compose coverage protects the filtering contract and gesture. The temporary backend, RSS server, and proxy were stopped, test data cleared, direct `5051` connectivity restored, and the crash buffer was empty. Full gate: 109 unit tests, 103/103 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 101/127 Verified (80%).

Scenario wave 18 completed on 2026-07-26 for `SUB-12` and `REL-13`. An actual isolated backend with two controlled feeds accepted Refresh all, left the visible library usable while its async job ran, and entered its production retry path when one feed failed. Existing polling/Compose coverage plus a new connected terminal-job regression require `failed`/`lastError` to stop the spinner, show the backend error, retain the loaded podcast/episode, and dispatch one job. The audit found that the initial Home and Subscriptions load could be duplicated by ViewModel initialization plus first lifecycle resume; both ViewModels now synchronously reject a concurrent refresh and immediate-double-call tests pin one request chain. A host-only proxy delayed exactly one real `GET /api/podcasts` to test backend `5051` for 35 seconds. Cold launch plus background/foreground showed only `Loading subscriptions`, no mutation actions, and no duplicate request; the 30-second deadline ended in `Could not load subscriptions.` with `Try again`. Passthrough restoration plus that real gesture recovered the authoritative Planet Money library. QA proxy/app state were cleared, direct `5051` restored, and the crash buffer was empty. Full gate: 109 unit tests, 106/106 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 103/127 Verified (81%).

Scenario wave 19 physical-device acceptance promoted `PLY-01`, `PLY-02`, `PLY-04`, `PLY-09`, `PLY-17`, `PLY-18`, `SET-10`, `SET-11`, and `REL-07`. Xiaomi `23021RAA2Y` on Android 15 ran the current test APK against backend `5051`. System Dark was followed on clean install; explicit Light/Dark persisted across force-stop. Real audio Play/Pause, backend active state, position retention, and a 75% absolute seek to authoritative position `1299` passed. Episode 16 completed naturally, became listened, left the queue, and episode 18 auto-started. Notification and lock-screen metadata were correct, but the phone exposed a real defect: Media3 also published Previous/Next although the product decision requires Play/Pause only. PlaybackService now restricts the media-notification controller command set, connected coverage protects it, and the rebuilt phone APK showed only Pause in both surfaces. A process/service stop restored episode 18 at confirmed backend position, paused and without autoplay. The 360-row Planet Money list remained scrollable and opened the intended actions for a deep long-title row. Audio-focus, route-disconnect, controlled stream failure, backup/logout, and production smoke remain separate open checks. Full gate: 109 unit tests, 107/107 connected tests, debug/release lint, debug/test APKs, and minified release APK. The scenario map is now 112/127 Verified (88%).

Scenario wave 20 completed `PLY-14` on the physical Android 15 phone. A restricted temporary proxy listened only on this Mac's LAN address, rejected every target except test backend `192.168.0.222:5051`, and returned `503` only for episode audio. Episode 18 failed after four real source requests; Home exposed the approved recoverable message and Play control, MediaSession reported a source error at retained `9:18`, and no queue/active state was corrupted. Restoring passthrough and tapping the error-state Play control cleared the error and resumed the same episode at 1.3x without app restart. Backend remained active on episode 18 with queue `[18,26]`; the phone proxy was reset to `:0` and the temporary process stopped. No production-code change was required. The scenario map is now 113/127 Verified (89%).

Scenario wave 21 completed `APP-12` and `PLY-15` on the physical phone. Logout removed mpod's MediaSession and opened Login; a force-stop/cold launch remained logged out. Clearing the test package and launching again also opened Login with no cookie/session preferences restored, while connected coverage protects the explicit backup/device-transfer exclusions. The agreed `t/123` test session was restored afterward. For real competing audio, mpod played episode 18 before Chrome played a LAN-only generated 20-second WAV. Chrome acquired full `AUDIOFOCUS_GAIN`; its MediaSession became playing and mpod immediately paused at retained position with no error. Pausing Chrome and returning to mpod did not incorrectly auto-resume after permanent focus loss. The audio fixture was stopped without force-stopping the user's Chrome. No production-code change was required. The scenario map is now 115/127 Verified (91%).

Production release preflight was repeated from Android HEAD `8bdb68d`. The minified APK is `app/build/outputs/apk/release/app-release.apk`, package `com.prod.mpod`, version `1.0.11 (12)`, min SDK 34, target SDK 36, and 5,879,637 bytes. SHA-256 is `d07037fe533835f9cf3a9c7322424164230969d5010bc9ba28e56fcee259da57`. Static inspection found `192.168.0.222:5050` and no test `5051` endpoint. APK Signature Scheme v2 verification passed for the local-network acceptance artifact. The APK installed on the physical Android 15 phone and reached production Login, confirming production-server connectivity. `REL-10` remains Specified until the product owner completes Login and the authenticated production smoke path.

Scenario wave 22 completed `PLY-16` on the physical phone. With `HL S3` active, Bluetooth reported A2DP playback, STREAM_MUSIC selected `bt_a2dp`, and mpod played episode 18 at 1.3x. The product owner physically disconnected the headphones. Android removed the active Bluetooth device and returned STREAM_MUSIC routing to speaker, while mpod immediately became PAUSED at retained position `799174`. Home exposed Play without a player error, so audio did not unexpectedly continue through the speaker and playback remained recoverable. No production-code change was required. The scenario map is now 116/127 Verified (91%).
