...
| SUB-12 | One feed fails during Refresh all | Other feeds finish successfully; partial failure is surfaced without breaking the library or recording a normal successful `Last refresh` result | C,U,E | Implemented — `341002d` passed code review; product retest was not reached after BUG-01 blocker and remains pending in `MPOD-QA-02-R1` |
