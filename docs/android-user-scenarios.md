...
| EPS-09 | Mark all listened for selected podcast | One consistent episode set is marked listened, removed from the playlist/queue, and scheduled for observable local-file cleanup without stale downloaded state during a concurrent feed refresh | C,U,E | Verified — 23.09.2026: concurrent refresh/download cleanup and repeated command checked through release UI and direct Room/files inspection; 19 JVM consistency tests and 8 Android transaction tests PASS. See [storage evidence](mpoddy-open-items-2026-09-23-report.md) and [technical gate](mpoddy-stage3-2026-09-23-report.md) |
