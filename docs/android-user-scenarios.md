...
| EV-W3-PARTIAL | 2026-07-19 | `SUB-03`, `SUB-04`, `SUB-12`, `SUB-14` remain Specified; partial evidence for `SUB-01`, `SUB-02` | Compose/state tests protected loading without mutation actions, load-error Retry, both empty-library add paths, distinct caught-up/Show all behavior, refresh failure UI, and scoped episode failure with another usable podcast. Complete `5051` evidence still required an isolated empty library, controlled initial-load/episode endpoint failures, and allowing backend Refresh all to exhaust its 30s/2m/5m feed retry schedule; shared Planet Money state was not destructively altered. `SUB-01` and `SUB-02` were subsequently promoted by `EV-W13` and `EV-W15` |
