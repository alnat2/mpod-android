...
| EV-W3-PARTIAL | 2026-07-19 | `SUB-12` remains Specified; partial evidence for `SUB-01`–`SUB-04`, `SUB-14` | Compose/state tests protected loading without mutation actions, load-error Retry, both empty-library add paths, distinct caught-up/Show all behavior, refresh failure UI, and scoped episode failure with another usable podcast. Complete evidence still required isolated backend states, controlled initial-load/episode endpoint failures, and allowing backend Refresh all to exhaust its retry schedule; shared Planet Money state was not destructively altered. `SUB-01`–`SUB-04` and `SUB-14` were subsequently promoted by `EV-W13`, `EV-W15`, `EV-W16`, and `EV-W17` |
